...
function App() {
  ...
  useEffect(() => {
    socket.on('newEarthquakes', (newEarthquakes) => {
      setEarthquakes(prev => [...newEarthquakes, ...prev.filter(eq => !newEarthquakes.some(n => n.id === eq.id))]);
      console.log('Received new earthquakes via Socket.IO:', newEarthquakes);
    });
    ...
    if ('serviceWorker' in navigator && 'PushManager' in window && VAPID_PUBLIC_KEY) {
      try {
        ...
        if (permission === 'granted') {
          ...
          const response = await axios.post(`${BASE_URL}/subscribe`, subscription);
          console.log('Subscription response:', response.data);
          setNotificationStatus('granted'); // Ensure state updates
          console.log('Subscribed to push notifications');
        } else {
          console.log('Notification permission denied');
          setNotificationStatus('denied'); // Ensure state updates
        }
      } catch (error) {
        console.error('Failed to enable notifications:', error);
        setNotificationStatus('denied'); // Set to denied on error
      }
    } else {
      console.log('Push notifications not supported in this browser');
    }
